...
export async function POST(req: Request) {
  ...
  let isNew: boolean;
  try {
    const existing = await prisma.workshopInterest.findUnique({
    ...
    await prisma.workshopInterest.upsert({
      where: { email: trimmedEmail },
      update: { firstName: trimmedFirstName, source: finalSource },
      create: {
    ...
    console.error("Workshop interest DB error:", error);
  ...
  // Emails only fire for genuinely new sign-ups.
  // Each is wrapped independently so a Resend failure never blocks the response.
  if (isNew) {
    try {
      ...
      console.error("Workshop interest admin email error:", error);
    }
    ...
    try {
      ...
      console.error("Workshop interest client email error:", error);
    }
  }
